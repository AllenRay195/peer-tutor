...
export default function TutorFocusCard({ focusSession }: Props) {
  const lastActive =
  ...
  return (
    ...
    >
      ...
      <p className="text-sm text-slate-600 mt-1">
        Student: {focusSession.studentName}
      </p>
      ...
      {goals.length > 0 ? (
        ...
        <div className="mt-4 rounded-lg border border-dashed bg-slate-50 p-3 text-sm text-slate-500">
          No goals set yet — align expectations early ✨
        </div>
      )}
      ...
      <div className="mt-5 flex flex-wrap gap-3">
        <Link
          href={`/dashboard/tutor/sessions/${focusSession.id}`}
          className="rounded-lg bg-indigo-600 px-4 py-2 text-sm text-white hover:bg-indigo-700 transition"
        >
          Open Session
        </Link>

        {!focusSession.hasSummary &&
          focusSession.status === "closed" && (
            <Link
              href={`/dashboard/tutor/sessions/${focusSession.id}/summary`}
